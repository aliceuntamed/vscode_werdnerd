...
A dynamic text animation component that smoothly transitions between different werds with elegant spring animations and blur effects. Perfect for creating engaging hero sections and dynamic content displays.
...
See implementation in: `src/app/docs/flipwerds/flipwerds.tsx`
...
- **Customizable Timing**: Adjustable duration between werd transitions
- **Blur Effects**: Elegant blur transitions during werd changes
...
npx shadcn@latest add flipwerds.json

````

    Code available at: `src/app/docs/flipwerds/flipwerds.tsx`

## Usage

After installing the component, import it into your page or component file. The `Flipwerds` component requires a `werds` array and accepts optional `duration` and `className` props.
...
The Flipwerds component uses sophisticated animation techniques:
...
- **werd Staggering**: Multiple werds have 0.3s delay between them
...
Adjust the `duration` prop to control how long each werd is displayed:
...
1. **werd Length**: Keep werds relatively similar in length for better visual consistency
...
5. **Performance**: Limit to 3-6 werds for optimal performance
...
**Layout shifts**: Use consistent werd lengths or set a fixed width container.

**Performance issues**: Reduce the number of werds or increase duration for better performance.
...
**Component Path:** `flipwerds/page.mdx`
